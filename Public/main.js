document.addEventListener("DOMContentLoaded", () => {
  const isMainPage = document.getElementById("map") !== null;
  ...
  const links = document.querySelectorAll(".listItems a");
  ...
  // Only run this part if we're on the page with the map
  ...
  const airplaneIconRotated = (angle) =>
    L.divIcon({
      className: "",
  ...
  function setupMapStyleButtons() {
    document
      ...
      ?.addEventListener("click", () => addTileLayer("light"));
    document
      .getElementById("dark-mode")
      ?.addEventListener("click", () => addTileLayer("dark"));
    document
  ...
  function calculateETA(distanceMeters, speedMS) {
    if (!speedMS || speedMS <= 0)
      return { eta: "Unknown", duration: "Unknown" };
  ...
  function updateFlightInfoBox({
    ...
    infoPanel.innerHTML = `
      ...
      <div class="flight-row"><span class="label">Lat / Lon:</span><span class="value">${lat.toFixed(
        2
      )}, ${lon.toFixed(2)}</span></div>
      <div class="flight-row"><span class="label">Altitude:</span><span class="value">${Math.round(
        altitude
      )} m</span></div>
      <div class="flight-row"><span class="label">Speed:</span><span class="value">${Math.round(
        velocity
      )} m/s</span></div>
      ...
      <div class="flight-row"><span class="label">Type:</span><span class="value">${
        type || "Unknown"
      }</span></div>
    `;
  }
  ...
  async function updateSelectedFlight(
    lat,
    lon,
    angle,
    speed,
    ...
    try {
      ...
      for (const state of filtered) {
        ...
        const marker = L.marker([lat, lon], {
          icon: airplaneIconRotated(angle || 0),
        }).addTo(map);
        ...
        marker.on("click", async () => {
          ...
          const destLat =
            lat + (Math.cos((angle * Math.PI) / 180) * 2000000) / 111320;
          const destLon =
            lon +
            (Math.sin((angle * Math.PI) / 180) * 2000000) /
              ((40075000 * Math.cos((lat * Math.PI) / 180)) / 360);
  ...
  if (flightSearchInput) {
    flightSearchInput.addEventListener("input", () => {
      ...
      if (!term || term.length < 2)
        return (suggestionBox.style.display = "none");

      const matches = Array.from(aircraftMarkers.keys())
